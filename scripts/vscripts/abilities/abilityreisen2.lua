...
function ability_thdots_reisen_2_02:Spawn()
    if not IsServer() then return end
    self.chargesRefreshed = false
    self.hasWanbaochui = false
end

function ability_thdots_reisen_2_02:OnInventoryContentsChanged()
    if not IsServer() then return end
	if self:GetCaster():HasModifier("modifier_item_wanbaochui") then
        self.hasWanbaochui = true
        if self.chargesRefreshed == false then
		    self:SetCurrentAbilityCharges(self:GetMaxAbilityCharges(self:GetLevel() - 1) - 1)
            if not self:IsCooldownReady() then
                self:EndCooldown()
                self:StartCooldown(self:GetCooldownTime())
            end
            self.chargesRefreshed = true
        end
    elseif self:GetCaster():HasModifier("modifier_item_wanbaochui") == false and self.hasWanbaochui == true then
        self.hasWanbaochui = false
        if self:GetCurrentAbilityCharges() == 0 then
            self:StartCooldown(self:GetEffectiveCooldown(self:GetLevel() - 1))
        end
	end
end
...
function modifier_ability_thdots_reisen2_02_buff_damageReduction:GetModifierIncomingDamage_Percentage(keys)
    ...
    --如果是塔和反伤直接 return
    if enemy:IsTower() or enemy:IsBuilding() or bit.band( keys.damage_flags, DOTA_DAMAGE_FLAG_REFLECTION ) == DOTA_DAMAGE_FLAG_REFLECTION then
