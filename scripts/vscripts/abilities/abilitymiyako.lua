...
local MIYAKO_EX_CHECK_INTERVAL = 0.5
local MIYAKO01_CASTER_INTERVAL = 0.2
local MIYAKO05_MOVE_INTERVAL = 0.04

local function Miyako01_SetTargetCount(caster, ability, count)
	if not ability then return end
	ability.miyako01_target_count = math.max(0, count or 0)
	if caster and not caster:IsNull() then
		local modifier = caster:FindModifierByName("modifier_ability_thdots_miyako01_caster")
		if modifier and modifier:GetStackCount() ~= ability.miyako01_target_count then
			modifier:SetStackCount(ability.miyako01_target_count)
		end
	end
end

local function Miyako01_AddTargetCount(caster, ability, delta)
	Miyako01_SetTargetCount(caster, ability, (ability and ability.miyako01_target_count or 0) + delta)
end

local function Miyako02_UpdateCasterBonus(caster, ability, delta)
	if not ability or not caster or caster:IsNull() or delta == 0 then return end
	ability.miyako02_bonus_total = math.max(0, (ability.miyako02_bonus_total or 0) + delta)
	local modifier = caster:FindModifierByName("modifier_ability_thdots_miyako02_caster")
	if modifier and modifier:GetStackCount() ~= ability.miyako02_bonus_total then
		modifier:SetStackCount(ability.miyako02_bonus_total)
	end
	if ability.miyako02_bonus_total > 0 then
		if not caster:HasModifier("modifier_ability_thdots_miyako02_caster_paticle") then
			caster:AddNewModifier(caster, ability, "modifier_ability_thdots_miyako02_caster_paticle", {})
		end
	else
...
function modifier_ability_thdots_miyakoEx_passive:OnCreated()
	if not IsServer() then return end
	self.last_is_daytime = nil
	if THD2_RefreshTalentModifiers ~= nil then
		THD2_RefreshTalentModifiers(self:GetCaster(), "ability_thdots_miyakoEx")
	end
	self:StartIntervalThink(MIYAKO_EX_CHECK_INTERVAL)
	self:OnIntervalThink()
end
function modifier_ability_thdots_miyakoEx_passive:OnIntervalThink()
	if not IsServer() then return end
	local is_daytime = GameRules:IsDaytime()
	if self.last_is_daytime ~= is_daytime then
...
function modifier_ability_thdots_miyako01_caster:OnCreated()
	...
	Miyako01_SetTargetCount(self.caster, self.ability, 0)
	self:StartIntervalThink(MIYAKO01_CASTER_INTERVAL)
	--特效
...
function modifier_ability_thdots_miyako01_caster:OnDestroy()
	...
	ParticleManager:ReleaseParticleIndex(self.miyako01_particle)
end

function modifier_ability_thdots_miyako01_caster:OnIntervalThink()
	if not IsServer() then return end
	-- 低频扣除维持消耗，层数由 target modifier 进入/离开时汇总。
	local mana_cost = self.caster:GetMaxMana() * 0.03 * MIYAKO01_CASTER_INTERVAL
	if self.caster:GetMana() < mana_cost then
		local miyako01 = self.caster:FindAbilityByName("ability_thdots_miyako01")
		if miyako01 ~= nil then
			miyako01:ToggleAbility()
		end
	end
	self.caster:SetMana(self.caster:GetMana() - mana_cost)
end
...
function modifier_ability_thdots_miyako01_target:OnCreated()
	...
	self.ability = self:GetAbility()
	self.counted = self.parent:IsRealHero()
	if self.counted then
		Miyako01_AddTargetCount(self.caster, self.ability, 1)
	end
...
function modifier_ability_thdots_miyako01_target:OnIntervalThink()
	...
	if self.parent:IsRealHero() then
		...
		self.parent:SetContextThink("miyako01_particle",function ()
			...
			ParticleManager:ReleaseParticleIndex(miyako01_particle)
		end, 0.2)
...
function modifier_ability_thdots_miyako02_caster:OnCreated()
	...
	self:GetAbility().miyako02_bonus_total = 0
...
function modifier_ability_thdots_miyako02_target:OnDestroy()
	if not IsServer() then return end
	Miyako02_UpdateCasterBonus(self.caster, self.ability, -self.add_bonus)
	self.add_bonus = 0
end
...
function modifier_ability_thdots_miyako02_target:OnIntervalThink()
	...
	--计算层数与百分比血量哪个多
	local old_bonus = self.add_bonus
	self.add_bonus = stack_count - hp_percent
	if self.add_bonus <= 0 then
		self.add_bonus = 0
	end
	Miyako02_UpdateCasterBonus(self.caster, self.ability, self.add_bonus - old_bonus)
end
...
function modifier_ability_thdots_miyako04_passive:OnDestroy()
	if not IsServer() then return end
	if self.particle_debuff_fx then
		ParticleManager:DestroyParticle(self.particle_debuff_fx, false)
		ParticleManager:ReleaseParticleIndex(self.particle_debuff_fx)
...
function modifier_ability_thdots_miyako04_active:OnDestroy()
	...
	ParticleManager:ReleaseParticleIndex(self.miyako04_particle)
end
--------------------------------------------------------
--僵尸「肉灵渴望」
--------------------------------------------------------
...
function modifier_ability_thdots_miyako05_none:OnDestroy()
	...
	ParticleManager:ReleaseParticleIndex(self.miyako05_particle)
end
...
function modifier_ability_thdots_miyako05_caster:OnCreated()
	...
	self.hit_targets = {}
	self:StartIntervalThink(MIYAKO05_MOVE_INTERVAL)
	--创建特效
	self.particle_name_1 = "particles/econ/items/juggernaut/jugg_arcana/juggernaut_arcana_v2_blade_fury_disk.vpcf"
	self.miyako05_particle = ParticleManager:CreateParticle(self.particle_name_1, PATTACH_CUSTOMORIGIN_FOLLOW,self.caster)
	-- ParticleManager:SetParticleControl(self.miyako05_particle, 5, Vector(250,1,1))
...
function modifier_ability_thdots_miyako05_caster:OnIntervalThink()
	...
	local distance_to_target = (caster_origin - self.point):Length2D()
	local vec = (self.point - caster_origin):Normalized()
	local frame_time = math.max(FrameTime(), 0.01)
	local move_distance = self.velocity * MIYAKO05_MOVE_INTERVAL / frame_time
	local hit_radius = 200 + move_distance
	--对沿途的单位造成一次普攻，以添加modifier的形式，同一次冲刺不重复命中。
	local targets = FindUnitsInRadius(self.caster:GetTeam(), self.caster:GetOrigin(),nil,hit_radius,self.ability:GetAbilityTargetTeam(),
		self.ability:GetAbilityTargetType(),0,0,false)
	for _,victim in pairs(targets) do
	...
	if distance_to_target <= 21 or distance_to_target <= move_distance then
		-- FindClearSpaceForUnit(self.caster,self.caster:GetOrigin(),true)
		self.caster:SetAbsOrigin(self.point)
		...
		self.caster:SetAbsOrigin(self.caster:GetAbsOrigin() + vec * move_distance) 
	end
end
