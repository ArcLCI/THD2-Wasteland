function OnMomiji01Spawn(keys)
    ...
    for i = 1, count do
        local unit = CreateUnitByName("ability_momiji_Spawn_unit", caster:GetOrigin() +
        ...
        unit:SetContextThink("ability_momiji01_Spawn_unit_regen", function()
...
function OnMomiji02Start(keys)
    ...
    AddFOWViewer(caster:GetTeam(), keys.ability:GetCursorPosition(), keys.Radius, Duration, false)

    local effectIndex = ParticleManager:CreateParticle("particles/econ/courier/courier_trail_05/courier_trail_05.vpcf",
    ...
    local effectIndex = ParticleManager:CreateParticle("particles/heroes/momiji/ping_waypoint_circle.vpcf",
        PATTACH_CUSTOMORIGIN, caster)
    ParticleManager:SetParticleControl(effectIndex, 0, keys.ability:GetCursorPosition())
    ParticleManager:SetParticleControl(effectIndex, 1, Vector(keys.Radius, 0, 0))
    ParticleManager:DestroyParticleSystemTime(effectIndex, Duration)

    local unit = CreateUnitByName("npc_vision_momiji_dummy_unit", caster:GetOrigin(), false, caster, caster,
        caster:GetTeam())

    unit:SetOrigin(keys.ability:GetCursorPosition())
    ...
    unit:AddNewModifier(caster, abilityGEM, "modifier_item_gem_of_true_sight", {
        duration = Duration
    })
    ...
    unit:SetContextThink("ability_momiji_02_vision", function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        unit:RemoveSelf()
...
function OnMomiji02IntervalThink(keys)
    ...
    for k, v in pairs(targets) do
        v:AddNewModifier(caster, keys.ability, "modifier_momiji02_debuff", {
            duration = 1
        })
    end
end
...
function modifier_momiji02_debuff:IsHidden()
    return false
end
function modifier_momiji02_debuff:IsDebuff()
    return true
end
function modifier_momiji02_debuff:IsPurgable()
    return true
end
...
function modifier_momiji02_debuff:OnAttackStart(keys)
    if IsServer() then
        ...
        if owner == keys.target then
            local result = RandomInt(0, 100)
            local chance = self:GetAbility():GetSpecialValueFor("crit_chance")
            local attacker = keys.attacker
            if attacker:GetPlayerOwnerID() ~= self:GetCaster():GetPlayerOwnerID() then
                chance = chance / 2;
            end
            local modifier = "modifier_momiji02_criticalstrike"
            if FindTelentValue(self:GetCaster(), "special_bonus_unique_momiji_4") ~= 0 then
                result = 1;
            end
            print(chance)
            if result <= chance then
                self:GetAbility():ApplyDataDrivenModifier(self:GetCaster(), attacker, modifier, {
                    duration = 1
                })
            end
        end
...
function OnMomiji03Start(keys)
    ...
    local effectIndex = ParticleManager:CreateParticle(
        "particles/units/heroes/hero_sven/sven_storm_bolt_projectile_explosion.vpcf", PATTACH_CUSTOMORIGIN, caster)
...
LinkLuaModifier("modifier_ability_thdots_momiji03_passvie", "scripts/vscripts/abilities/abilityMomiji.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_ability_thdots_momiji03_passvie:IsHidden()
    return false
end
function modifier_ability_thdots_momiji03_passvie:IsPurgable()
    return false
end
function modifier_ability_thdots_momiji03_passvie:RemoveOnDeath()
    return false
end
function modifier_ability_thdots_momiji03_passvie:IsDebuff()
    return false
end
...
function modifier_ability_thdots_momiji03_passvie:GetModifierTotal_ConstantBlock(kv)
    ...
    local targetPoint = vecCaster + caster:GetForwardVector() -- keys.ability:GetCursorPosition()
...
function passive_momiji04_bonus:IsHidden()
    return false
end
function passive_momiji04_bonus:IsPurgable()
    return false
end
function passive_momiji04_bonus:RemoveOnDeath()
    return false
end
function passive_momiji04_bonus:IsDebuff()
    return false
end

function passive_momiji04_bonus:IsAura()
    return true
end
...
function passive_momiji04_bonus:GetAuraSearchTeam()
    return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end
function passive_momiji04_bonus:GetAuraSearchType()
    return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC
end
function passive_momiji04_bonus:GetModifierAura()
    return "modifier_momiji04_bonus"
end
...
function modifier_momiji04_bonus:IsHidden()
    return false
end
function modifier_momiji04_bonus:IsPurgable()
    return false
end
function modifier_momiji04_bonus:RemoveOnDeath()
    return true
end
function modifier_momiji04_bonus:IsDebuff()
    return false
end
...
function ability_thdots_momiji04:OnSpellStart()
    ...
    for _, v in pairs(targets) do
        if v:IsRealHero() then
            print(v:GetName())
            v:AddNewModifier(caster, self, "modifier_momiji04_wanbaochui_bonus", {
                duration = duration
            })
        end
    end
...
LinkLuaModifier("modifier_momiji04_wanbaochui_bonus", "scripts/vscripts/abilities/abilityMomiji.lua",
    LUA_MODIFIER_MOTION_NONE)
function modifier_momiji04_wanbaochui_bonus:IsHidden()
    return false
end
function modifier_momiji04_wanbaochui_bonus:IsPurgable()
    return true
end
function modifier_momiji04_wanbaochui_bonus:RemoveOnDeath()
    return true
end
function modifier_momiji04_wanbaochui_bonus:IsDebuff()
    return false
end
...
function modifier_momiji04_wanbaochui_bonus:OnCreated()
    ...
    self.particle_stampede_fx = ParticleManager:CreateParticle(self.particle_stampede, PATTACH_ABSORIGIN_FOLLOW,
        self:GetParent())
