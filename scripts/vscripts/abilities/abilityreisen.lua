...
function OnReisenOld02SpellSuccess(keys)
    ...
    if (caster:HasModifier("modifier_illusion")) then
        ...
        if not caster:GetContext("ReisenOld02IllusionCaster") then
            return
        end
...
function OnReisenOld04SpellStart(keys)
    ...
    keys.ability:ApplyDataDrivenModifier(caster, caster, "ability_reisenold04_modifier", {
        Duration = keys.duration + FindTelentValue(caster, "special_bonus_unique_reisen_2")
    })
    keys.ability:ApplyDataDrivenModifier(caster, caster, "ability_reisenold04_bouns_attack_speed", {
        Duration = keys.duration + FindTelentValue(caster, "special_bonus_unique_reisen_2")
    })
end
