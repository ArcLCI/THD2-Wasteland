...
function ability_thdots_chen01:GetCastRange(location, target)
...
function ability_thdots_chen02:OnSpellStart()
	...
	self.target:AddNewModifier(self.caster, self, "modifier_ability_thdots_chen02_target", {duration = self.wait_time + 0.05})
...
function modifier_ability_thdots_chen02_target:OnAbilityExecuted(keys)
	if not IsServer() then return end
	if keys.unit ~= self.target or keys.ability:IsItem() or IsNotLunchbox_ability(keys.ability) then
		return
	end
	self:SetStackCount(1)
end
...
function ability_thdots_chen_wanbaochui:OnSpellStart()
	local ability1 = self:GetCaster():GetAbilityByIndex(0)
	print(ability1:GetName())
	local ability2 = self:GetCaster():GetAbilityByIndex(1)
	print(ability2:GetName())
	local ability3 = self:GetCaster():GetAbilityByIndex(2)
	print(ability3:GetName())
