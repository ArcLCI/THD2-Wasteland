...
--]] function OnPatchouliAddFire(keys)
    local caster = keys.caster
    if caster.ElementsNum == nil then
        caster.ElementsNum = 1
    end
    if caster.ElementsName == nil then
        caster.ElementsName = {}
    end
...
function OnPatchouliInvoke(keys)
    ...
    if caster.SwapAbilityName == nil then
        caster.SwapAbilityName = "ability_thdots_patchouli_xianzhezhishi"
    end
    ...
    elseif caster:HasModifier("modifier_thdots_patchouli_fire") and
        caster:HasModifier("modifier_thdots_patchouli_metal") then
        caster.NewAbilityName = "ability_thdots_patchouli_fire_metal"
    elseif caster:HasModifier("modifier_thdots_patchouli_fire") and
        caster:HasModifier("modifier_thdots_patchouli_earth") then
        caster.NewAbilityName = "ability_thdots_patchouli_fire_earth"
    elseif caster:HasModifier("modifier_thdots_patchouli_water") and
        caster:HasModifier("modifier_thdots_patchouli_wood") then
    ...
    elseif caster:HasModifier("modifier_thdots_patchouli_wood") and
        caster:HasModifier("modifier_thdots_patchouli_metal") then
        caster.NewAbilityName = "ability_thdots_patchouli_wood_metal"
    elseif caster:HasModifier("modifier_thdots_patchouli_wood") and
        caster:HasModifier("modifier_thdots_patchouli_earth") then
        caster.NewAbilityName = "ability_thdots_patchouli_wood_earth"
    elseif caster:HasModifier("modifier_thdots_patchouli_metal") and
        caster:HasModifier("modifier_thdots_patchouli_earth") then
...
function OnPatchouliFireWater(keys)
    ...
    local time = 0
    target:SetContextThink(DoUniqueString("OnPatchouliFireWaterThink"), function()
        ...
        for _, v in pairs(targets) do
            ...
            if v and not v:IsNull() then
                keys.ability:ApplyDataDrivenModifier(caster, v, "modifier_thdots_patchouli_fire_water", {
                    duration = 0.5
                })
            end
        end
    ...
    keys.ability:ApplyDataDrivenModifier(caster, target, "modifier_thdots_patchouli_fire_water", {
...
function OnPatchouliFireWood(keys)
    ...
    local time = 0
    GameRules:GetGameModeEntity():SetContextThink(DoUniqueString("OnPatchouliFireWood"), function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
...
function OnPatchouliFireMetal(keys)
    ...
    if target:GetTeam() == caster:GetTeam() then
        ...
        target:SetModifierStackCount("modifier_patchouli_fire_metal_bonus_damage", keys.ability,
            firelevel * 15 + FindTelentValue(caster, "special_bonus_unique_patchouli_1"))
        target:SetModifierStackCount("modifier_patchouli_fire_metal_bonus_attack_speed", keys.ability,
            metallevel * 15 + FindTelentValue(caster, "special_bonus_unique_patchouli_1"))
        keys.ability:ApplyDataDrivenModifier(caster, target, "modifier_patchouli_fire_metal_effect", {})
    else
        if is_spell_blocked(keys.target) then
            return
        end
        local deal_mana_damage = keys.ManaDamage + metallevel * keys.MetalMultiple + target:GetMana() *
                                     (keys.BaseManaDamage)
        ...
        target:Script_ReduceMana(deal_mana_damage, keys.ability)
        local digits = string.len(math.floor(deal_mana_damage)) + 1
        local numberIndex = ParticleManager:CreateParticle(
            "particles/units/heroes/hero_nyx_assassin/nyx_assassin_mana_burn_msg.vpcf", PATTACH_OVERHEAD_FOLLOW, target)
        ...
        Timer.Wait 'patchouli_fire_metal'(2.0, function()
...
function OnPatchouliFireEarth(keys)
    ...
    local time = 0
    GameRules:GetGameModeEntity():SetContextThink(DoUniqueString("OnPatchouliFireEarthEffect"), function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
...
function CheckPosition(keys)
    ...
    if distance_from_border < 0 and math.abs(distance_from_border) <= 20 and
        (math.abs(target_angle - angle_from_center) < 90 or math.abs(target_angle - angle_from_center) > 270) then
        -- Removes the movespeed minimum
        if target:HasModifier("modifier_movespeed_cap_low") == false then
            target:AddNewModifier(caster, nil, "modifier_movespeed_cap_low", {
                duration = deal_duration
            })
        end
    ...
    elseif distance_from_border > 0 and math.abs(distance_from_border) <= 30 and
        (math.abs(target_angle - angle_from_center) > 90) then
        -- Removes the movespeed minimum
        if target:HasModifier("modifier_movespeed_cap_low") == false then
            target:AddNewModifier(caster, nil, "modifier_movespeed_cap_low", {
                duration = deal_duration
...
function OnPatchouliFireFire(keys)
    ...
    for i = 1, count do
        local unit = CreateUnitByName("npc_thdots_unit_patchouli_fire_fire", caster:GetOrigin() +
            (caster:GetForwardVector() + Vector(math.cos((i - 1.5) * math.pi / 3), math.sin((i - 1.5) * math.pi / 3), 0)) *
            100, false, caster, caster, caster:GetTeam())

        if unit == nil then
            return
        end
        keys.ability:ApplyDataDrivenModifier(caster, unit, "modifier_thdots_patchouli_fire_fire", {
...
function OnPatchouliFireFireUnit(keys)
    if keys.unit:IsBuilding() == false then
        return
    end
    local caster = EntIndexToHScript(keys.caster_entindex)
...
function OnPatchouliWaterMetal(keys)
    ...
    local time = 0
    GameRules:GetGameModeEntity():SetContextThink(DoUniqueString("OnPatchouliWaterMetal"), function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        ...
        for _, v in pairs(targets) do
            ...
            v:AddNewModifier(caster, keys.ability, "modifier_patchouli_water_metal_debuff", {
                duration = 1.0
            })
            UnitDamageTarget(damage_table)
        end
...
LinkLuaModifier("modifier_patchouli_water_metal_debuff", "scripts/vscripts/abilities/abilitypatchouli.lua",
    LUA_MODIFIER_MOTION_NONE)
modifier_patchouli_water_metal_debuff = {}

function modifier_patchouli_water_metal_debuff:IsPurgable()
    return true
end
function modifier_patchouli_water_metal_debuff:IsPurgeException()
    return false
end
function modifier_patchouli_water_metal_debuff:RemoveOnDeath()
    return false
end
function modifier_patchouli_water_metal_debuff:IsHidden()
    return false
end
function modifier_patchouli_water_metal_debuff:IsDebuff()
    return true
end
function modifier_patchouli_water_metal_debuff:GetAttributes()
    return MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE
end

function modifier_patchouli_water_metal_debuff:OnCreated(keys)
    local abilitywater = self:GetCaster():FindAbilityByName("ability_thdots_patchouli_water")
    local waterlevel = abilitywater:GetLevel()
    local baseResistanceBonus = self:GetAbility():GetSpecialValueFor("reduce_resistance")
    local waterMulti = self:GetAbility():GetSpecialValueFor("water_multiple")
    self.value = -(baseResistanceBonus + waterlevel * waterMulti)
    self:SetHasCustomTransmitterData(true)
end

function modifier_patchouli_water_metal_debuff:DeclareFunctions()
    return {MODIFIER_PROPERTY_MAGICAL_RESISTANCE_BONUS}
end

function modifier_patchouli_water_metal_debuff:GetModifierMagicalResistanceBonus()
    return self.value
end

function modifier_patchouli_water_metal_debuff:AddCustomTransmitterData()
    return {
        resisctanceBonus = self.value
    }
end
function modifier_patchouli_water_metal_debuff:HandleCustomTransmitterData(data)
    self.value = data.resisctanceBonus
end
...
function OnPatchouliWaterEarthKnockback(keys, target, vecDirection)
    ...
    Ability:ApplyDataDrivenModifier(Caster, target, keys.knockback_debuff_name, {
        duration = duration
    })
    target:SetContextThink("knockback", function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
...
function OnPatchouliWaterEarth(keys)
    ...
    if Caster:HasModifier("modifier_item_wanbaochui") == false then
        ...
        Caster:SetContextThink("water_earth_main_loop", function()
            if GameRules:IsGamePaused() then
                return 0.03
            end
            if Ability:IsNull() then
            ...
            local enemies = FindUnitsInRadius(Caster:GetTeamNumber(), VecPos, nil, keys.hitbox_radius,
            ...
            for _, v in pairs(enemies) do
                if v:HasModifier("modifier_thdots_patchouli_water_earth_lock") == false then
                    ...
                    Ability:ApplyDataDrivenModifier(Caster, v, "modifier_thdots_patchouli_water_earth_lock", {
                        duration = (keys.range + keys.ExRange * 7) / keys.speed
                    })
                    UnitDamageTarget(damage_table)
                end
            end
        ...
        while (i < 13) do
            ...
            local TargetPoint = Vector(VecStart.x + 400 * math.sin(angel), VecStart.y + 400 * math.cos(angel),
                VecStart.z)
            local Direction = (TargetPoint - VecStart):Normalized()
            local TickInterval = keys.tick_interval
            local MovePerTick = keys.speed * TickInterval
            ...
            Caster:SetContextThink(DoUniqueString("water_earth_main_loop"), function()
                if GameRules:IsGamePaused() then
                    return 0.03
                end
                if Ability:IsNull() then
...
function OnPatchouliWaterWater(keys)
    ...
    GameRules:GetGameModeEntity():SetContextThink(DoUniqueString("OnPatchouliWaterWater"), function()
        ...
        DOTA_UNIT_TARGET_TEAM_ENEMY, keys.ability:GetAbilityTargetType(), 0, FIND_CLOSEST, false)
        for _, v in pairs(targets) do
            keys.ability:ApplyDataDrivenModifier(caster, v, "modifier_patchouli_water_water_debuff", {
                duration = 0.1
            })
            keys.ability:ApplyDataDrivenModifier(caster, v, "modifier_patchouli_water_water_debuff_01", {
                duration = 0.1
            })
            keys.ability:ApplyDataDrivenModifier(caster, v, "modifier_patchouli_water_water_debuff_02", {
                duration = 0.1
            })
...
function OnPatchouliWoodMetal(keys)
    ...
    Caster:SetContextThink("wood_metal_main_loop", function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        if Ability:IsNull() then
...
function OnPatchouliWoodEarth(keys)
    ...
    keys.ability:ApplyDataDrivenModifier(caster, target, "modifier_patchouli_wood_earth_root", {
        duration = Duration
    })
    wood_earth_effectIndex = ParticleManager:CreateParticle(
        "particles/units/heroes/hero_treant/treant_overgrowth_vines.vpcf", PATTACH_CUSTOMORIGIN, nil)
    ParticleManager:SetParticleControl(wood_earth_effectIndex, 0, Vec)
    local time = 0
    GameRules:GetGameModeEntity():SetContextThink(DoUniqueString("OnPatchouliWoodEarth"), function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
...
function OnPatchouliMetalEarth(keys)
    ...
    local deal_damage = keys.Damage + metallevel * keys.ExDamage +
                            FindTelentValue(Caster, "special_bonus_unique_patchouli_9")
    ...
    Caster:SetContextThink("metal_earth_main_loop", function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        if Ability:IsNull() then
        ...
        for _, v in pairs(enemies) do
            if v:HasModifier("modifier_thdots_patchouli_metal_earth_lock") == false then
                ...
                Ability:ApplyDataDrivenModifier(Caster, v, "modifier_thdots_patchouli_metal_earth_lock", {
                    duration = 0.5
                })
                UnitDamageTarget(damage_table)
            end
...
function OnPatchouliWoodWood(keys)
    ...
    local time = 0
    target:SetContextThink("wood_wood_main_loop", function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
        if time < deal_duration then
            keys.ability:ApplyDataDrivenModifier(caster, target, "modifier_thdots_patchouli_wood_wood_effect", {
                duration = deal_duration
            })
        else
            return nil
...
function OnPatchouliEarthEarth(keys)
    ...
    GameRules:GetGameModeEntity():SetContextThink(DoUniqueString("OnPatchouliEarthEarth"), function()
        if GameRules:IsGamePaused() then
            return 0.03
        end
...
function MetalMetalOnCreated(keys)
    ...
    local effectIndex = ParticleManager:CreateParticle(
        "particles/units/heroes/hero_dark_willow/dark_willow_wisp_spell_fear_debuff.vpcf", PATTACH_ABSORIGIN_FOLLOW,
        caster)
    ParticleManager:DestroyParticleSystemTime(effectIndex, FindValueTHD("duration", ability))
    keys.ability:ApplyDataDrivenModifier(caster, target, "modifier_patchouli_metal_metal", {
        duration = 10.0
    }) -- 野蛮
end
...
function OnPatchouliMetalMetal(keys)
    ...
    if ability.origin ~= null then
        ...
        if distance <= damage_cap_amount and distance > 0 then
            local damage = distance * (movement_damage_pct + keys.ExDamage * metallevel) / 100
            local damage_table = {
                ability = keys.ability,
                victim = target,
                attacker = caster,
...
function OnPatchouliXianzhezhishi(keys)
    ...
    if caster:HasModifier("modifier_thdots_patchouli_xianzhezhishi_add_cd") then
        return
    end
    if caster:HasModifier("modifier_thdots_patchouli_xianzhezhishi_add") == false then
    ...
    if FindTelentValue(caster, "special_bonus_unique_patchouli_6") == 1 then
        ability_xianzhezhishi:ApplyDataDrivenModifier(caster, caster, "modifier_thdots_patchouli_xianzhezhishi_add", {
            Duration = 4.75
        })
    else
        ability_xianzhezhishi:ApplyDataDrivenModifier(caster, caster, "modifier_thdots_patchouli_xianzhezhishi_add", {
    ...
    caster:SetModifierStackCount("modifier_thdots_patchouli_xianzhezhishi_add", ability_xianzhezhishi,
        caster.stone_count)
    ability_xianzhezhishi:ApplyDataDrivenModifier(caster, caster, "modifier_thdots_patchouli_xianzhezhishi_add_cd", {
        Duration = 0.25
    })
    if caster.stone_count == 5 then
        ...
        if FindTelentValue(caster, "special_bonus_unique_patchouli_8") == 1 then
            ability_xianzhezhishi:ApplyDataDrivenModifier(caster, caster,
            ...
            local time = 0
            GameRules:GetGameModeEntity():SetContextThink(DoUniqueString("OnPatchouliExSunAndMoon"), function()
                if GameRules:IsGamePaused() then
                    return 0.03
                end
                if time == 0 then
...
function OnPatchouliSunMoon(keys)
    ...
    if FindTelentValue(caster, "special_bonus_unique_patchouli_7") == 1 then
        ability:ApplyDataDrivenModifier(caster, caster, "modifier_thdots_patchouli_xianzhezhishi_sun", {
            duration = -1
        })
        ability:ApplyDataDrivenModifier(caster, caster, "modifier_thdots_patchouli_xianzhezhishi_moon", {
            duration = -1
        })
    else
        if GameRules:IsDaytime() then
...
function OnPatchouliRoyalFlame(keys)
    ...
    local effectIndex1 = ParticleManager:CreateParticle(
...
function OnPatchouliMoonDivinity(keys)
    ...
    local vec_caster = caster:GetOrigin()
    local effectIndex1 = ParticleManager:CreateParticle(
        "particles/units/heroes/hero_luna/luna_eclipse_cast_moonlight_glow03.vpcf", PATTACH_CUSTOMORIGIN, caster)
    ParticleManager:SetParticleControlEnt(effectIndex1, 0, caster, 5, "attach_hitloc", Vector(0, 0, 0), true)
    ...
    nil, -- find entity
    1200, -- find radius
    DOTA_UNIT_TARGET_TEAM_FRIENDLY, keys.ability:GetAbilityTargetType(), 0, FIND_CLOSEST, false)
    for _, v in pairs(targets) do
        local effectIndex = ParticleManager:CreateParticle(
            "particles/units/heroes/hero_mirana/mirana_moonlight_ray.vpcf", PATTACH_CUSTOMORIGIN, v)
        ParticleManager:SetParticleControl(effectIndex, 0, v:GetOrigin())
