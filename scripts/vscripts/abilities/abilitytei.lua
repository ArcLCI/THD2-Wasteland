...
function Tei03OnDestroy( keys )
	local caster = keys.caster
	print("caster:GetName(): "..caster:GetName())
	if caster:GetName() == "npc_dota_hero_gyrocopter" then
...
function Tei04OnAttackLanded( keys )
	...
	if target:IsHero() then
		if FindTelentValue(caster,"special_bonus_unique_tei_2")==0 then
			keys.ability:ApplyDataDrivenModifier(caster, target, "modifier_ability_thdots_tei04_armor_reduce",{Duration = FindValueTHD("duration",keys.ability)})
		else
			keys.ability:ApplyDataDrivenModifier(caster, target, "modifier_ability_thdots_tei04_armor_reduce_talent",{Duration = FindValueTHD("duration",keys.ability)})
		end
		local count
