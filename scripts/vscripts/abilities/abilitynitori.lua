...
function ability_thdots_nitori02:OnProjectileHit(target, location)
	...
	if IsServer() then
		if target then
			...
			self.damage = self:GetSpecialValueFor("damage") + (self:GetCaster():GetIntellect(false) * self.intellect_bonus) * self.num
...
function ability_thdots_nitori04:OnSpellStart()
	...
	for i = 1,number do
		...
		self.tNitori04Funnel[i].Funnel:FindAbilityByName("ability_dummy_unit"):SetLevel(1)
...
function modifier_ability_thdots_nitori04_funnel_cast:OnDestroy()
	...
	local damage = self:GetAbility():GetSpecialValueFor("damage") + self:GetCaster():GetIntellect(false)*self:GetAbility():GetSpecialValueFor("int_bonus") --智力加成
